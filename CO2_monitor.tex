...
\lstinputlisting[firstline=34, lastline=100, caption={Load required libraries and set some configurations.}, label={lst:include}]%
...
\lstinputlisting[firstline=102, lastline=128, caption={Prepare website.}, label={lst:prepare_website}]%
{code/CO2_monitor.ino}
...
The function to print the data obtained from the SCD30 to the serial console is shown in Listing~\ref{lst:printSerial}, the function to print the data to the OLED display is given in Listing~\ref{lst:printOLED}.
\lstinputlisting[firstline=131, lastline=139, caption={Function which prints data to the serial console.},label={lst:printSerial}]%
{code/CO2_monitor.ino} 
\lstinputlisting[firstline=142, lastline=169, caption={Function which prints data to the OLED display.}, label={lst:printOLED}]%
{code/CO2_monitor.ino} 

Depending on the the \coo concentration, the OLED display shows an emoji with the level of happiness being correlated to the \coo concentration. The corresponding function to draw that face is given in Listing~\ref{lst:printOLED}.
\lstinputlisting[firstline=172, lastline=306, caption={Function which prints smilies to the OLED display depending on the \coo concentration.}, label={lst:printOLED}]%
{code/CO2_monitor.ino} 
...
The calibration and setup of the SCD30 is put into a separate function, shown in Listing~\ref{lst:setup}. An additional function, given in Listing~\ref{lst:SCD30_FRC}, performs a forced recalibration of the SCD30.
\lstinputlisting[firstline=350, lastline=385, caption={Setup code for the SCD30.}, label={lst:setup}]%
{code/CO2_monitor.ino}
\lstinputlisting[firstline=309, lastline=347, caption={Function to perform a forced recalibration of the SCD30.}, label={lst:SCD30_FRC}]%
{code/CO2_monitor.ino}
...
\lstinputlisting[firstline=483, lastline=536, caption={Main loop which is executed repeatedly.}, label={lst:main}]%
{code/CO2_monitor.ino} 
